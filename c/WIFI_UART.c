...
void set_properties_deal(UI08 buf)
{
    ...
    else if(SIID_data=='2')//SIID为2
    {
          ...
          case SET_POWER://开关机
	  {
	     ...
	     {SET_POWER_Err=1;return;}//错误指令 回复-4003
    ...
    else if(SIID_data=='6')//SIID为6
    {
      if(type_buf==SET_CHILD_LOCK)
      {
      	    ...
      	    if(chack_down_deal(key_Water_Full_bit))
	    {
	     ...
	     {SET_CHILD_LOCK_Err=1;return;}//识别不了的指令
...
void  get_properties_deal(void)
{
   ...
   for(i=0;i<9;i++)
   {Txd_props_bit[i].lword=0;}//清除所有发送标志
   //
...
void Wifi_Rxd_Deal(void)
{
   ...
   ||(Txd_props_ok_bit[1].lword)
   ||(Txd_props_ok_bit[2].lword)
   ||(Txd_props_ok_bit[3].lword)
   ||(Txd_props_ok_bit[4].lword)
   ||(Txd_props_ok_bit[5].lword)
   ||(Txd_props_ok_bit[6].lword)
   ||(Txd_props_ok_bit[7].lword)
   ||(Txd_props_ok_bit[8].lword))
   {
...
void Wifi_Txd_Deal(void)
{
   ...
   */
   ...
   ||(Txd_props_bit[1].lword)
   ||(Txd_props_bit[2].lword)
   ||(Txd_props_bit[3].lword)
   ||(Txd_props_bit[4].lword)
   ||(Txd_props_bit[5].lword)
   ||(Txd_props_bit[6].lword)
   ||(Txd_props_bit[7].lword)
   ||(Txd_props_bit[8].lword))
   {
       addr_point=0;
