...
MCU_xdata bool_f sys_reg__ec_60 = FALSE; //EC保护60分钟
MCU_xdata bool_f sys_reg__comp_protect_stause_condition_a = FALSE;//冷媒泄漏条件2中条件a压缩机保护
MCU_xdata bool_f sys_reg__comp_protect_stause_condition_b = FALSE;//冷媒泄漏条件2中条件b压缩机保护
MCU_xdata UI16 sys_reg_comp_38_count_condition_a = 0;//冷媒泄漏条件2中条件a时间计数
MCU_xdata UI16 sys_reg_comp_38_count_condition_b = 0;//冷媒泄漏条件2中条件b时间计数

MCU_xdata UI08 g_ec_count = 0;   //EC计时变量
MCU_xdata UI16 g_ec_count_timer = 0;//EC计时变量
...
void def_start_jude(bool_f condition1,bool_f condition2)
{
   ...
   //condition2 30分钟计时中途清零条件
     ...
     //
     if((Run_Reg.temp_room_point<defrost_para7)
     ||(Run_Reg.sensor_err))
     {
        Run_Reg.def_type=TYPE_B;a=19*60;
     }
     else
     {
        Run_Reg.def_type=TYPE_A;a=24*60;
     }
     //
