...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_DEHUMIDIFY_SET_VALUE,当前目标湿度); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_MODE,当前模式); //枚举型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_INDOOR,当前室内湿度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_CHILD_LOCK,当前童锁); //BOOL型数据上报;
    mcu_dp_fault_update(DPID_FAULT,当前故障告警); //故障型数据上报;
    mcu_dp_bool_update(DPID_TANK_FULL,当前水箱水位); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_DEFROST_STATUS,当前除霜状态); //BOOL型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_dehumidify_set_value_handle
功能描述 : 针对DPID_DEHUMIDIFY_SET_VALUE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
